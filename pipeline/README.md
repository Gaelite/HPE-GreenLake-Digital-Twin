...
### Prerequisites
```bash
pip install boto3 pandas pyarrow supabase
aws configure
```
...
```sql
-- Vehicles by type with average risk score
...
```bash
aws s3 ls | grep emergency-vehicles
```

### Issue: Crawler finds no tables
**Solution**: Check Parquet files exist in S3
```bash
aws s3 ls s3://emergency-vehicles-processed/processed/ --recursive
```
